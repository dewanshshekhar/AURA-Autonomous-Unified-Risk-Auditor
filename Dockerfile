...
RUN echo 'from llama_cpp import llama_cpp\nhas_cublas = hasattr(llama_cpp, "_LLAMA_CUBLAS")\ncuda_enabled = has_cublas and llama_cpp._LLAMA_CUBLAS\nprint(f"CUDA support enabled: {cuda_enabled}")\nif not cuda_enabled:\n    print("WARNING: llama-cpp-python was not compiled with CUDA support. Use quick_cuda_fix.sh to enable it.")' > /tmp/check_cuda.py && \
    python /tmp/check_cuda.py || echo "CUDA check completed (build continues regardless)"
...
RUN mkdir -p /app/AVAI/config && \
    ...
    max_tokens = 2048\n\
    temperature = 0.0\n\
    n_gpu_layers = -1' > /app/AVAI/config/config.toml

# Copy rest of the code
COPY . .

# Create scripts directory and GPU diagnostic script
RUN mkdir -p /app/AVAI/scripts
...
RUN echo '#!/bin/bash\n\
    ...
    exit 1\n\
    fi' > /app/AVAI/download_vision_model.sh && \
    chmod +x /app/AVAI/download_vision_model.sh
...
RUN echo '#!/bin/bash\n\
    ...
    /app/AVAI/download_vision_model.sh\n\
    if [ $? -ne 0 ]; then\n\
